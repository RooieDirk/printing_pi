...
Dlg::Dlg(wxWindow* parent, wxWindowID id, const wxString& title,
  ...
  if (pConf) {
    ...
    pConf->Read("printingUseAis", &m_bUseAis, 0);
    pConf->Read("printingUseFile", &m_bUseFile, 0);
    pConf->Read("printingMMSI", &m_tMMSI, "123456789");
  }
}
...
void Dlg::StartDriving() {
  ...
  if (m_bUseFile) {
    ...
    if (filedlg.ShowModal() != wxID_OK) {
      wxMessageBox(_("Printing has been stopped"));
      return;
    } else {
...
void Dlg::OnSART(wxCommandEvent& event) {
  if (m_Timer->IsRunning()) {
    ...
    wxMessageBox(_("Printing has not been started"));
}
...
void Dlg::OnWind(wxCommandEvent& event) {
  ...
  if (!m_bPrintingHasStarted) {
    wxMessageBox(_("Please start Printing"));
    return;
  }
...
double Dlg::ReadPolars(wxString filename, double windangle, double windspeed) {
  ...
  if (!doc.LoadFile(filename.mb_str())) {
    ...
    if (strcmp(root.Element()->Value(), "Printing")) {
      m_bInvalidPolarsFile = true;
      return -1;
    }
...
wxString Dlg::StandardPath() {
  ...
  if (wxDirExists(oldPath) && !wxDirExists(stdPath)) {
    wxLogMessage("Printing_pi: moving config dir %s to %s", oldPath, stdPath);
